...
export default function WalletConnector() {
  ...
  const { manualPublicKey, manualPublicKeyString, setManualPublicKeyString } =
      ...
      {!connected ? (
        <>
          ...
          <div className="w-full lg:w-4/5 text-solblue-dark">
            <div className="border rounded-md border-gray-800 dark:border-solblue">
              <input
                className="w-full h-7 px-5 placeholder-gray-800 dark:placeholder-solblue border-none bg-transparent dark:text-solblue"
                type="text"
                placeholder="Public Key / Wallet Address"
                value={manualPublicKeyString}
                onChange={(event) =>
                  setManualPublicKeyString(event.target.value)
                }
              />
            </div>
            <span
              hidden={
                manualPublicKeyString === '' || manualPublicKey !== undefined
              }
        ...
        <div className="solBoxBlue w-full font-light flex flex-wrap justify-between items-center">
          <div className="pl-5 pt-1">
            <span>
              <svg
                ...
                className="h-5 w-5 mb-0.5 mr-1.5 inline-block"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
          ...
          <div className="w-full lg:w-2/5 p-3 text-center md:text-right">
            <button className="solBtnGray" onClick={disconnect}>
              Disconnect
            </button>
          </div>
        </div>
